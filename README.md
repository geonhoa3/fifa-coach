...
- **문제** — 중계 화면에서 선수·공·골대를 직접 인식하려면 대규모 라벨링과 3D tactical 모델이 필요. 스포츠 분석 기업이 팀 단위로 푸는 문제라 개인이 정공법으로 붙으면 비현실적.
- **판단** — FC 온라인 미니맵엔 22명의 위치와 공이 이미 **2D로 추상화**되어 있다. *인식할* 필요 없이 *읽으면* 된다.
- **결과** — 3D 인식·ML 학습·데이터 라벨링을 **전부 생략**하고 OpenCV + 통계만으로 게임 상태를 추출. **영상 1개만 있으면 작동** → 수개월치 데이터·모델 학습 비용을 0으로 만들고, 기업 스케일 문제를 개인 프로젝트로 끌어내렸다. *"어려운 문제를 잘 푼 게 아니라, 안 풀어도 되게 우회한"* 설계 결정.

### 2. HSV 컬러 마스킹 + 컨투어 검출로 오브젝트 위치 인식
- 미니맵의 팀별 dot을 **HSV 색공간 임계값**으로 분리하고 **컨투어 검출**로 공·조작 선수·양팀 위치를 프레임 단위로 추출.
- 팀 색이 반전되는 **빨강 유니폼 케이스를 dot 검출률로 자동 감지·분기**. → 이 예외를 놓치면 팀 판정이 뒤집혀 **그 경기 분석이 통째로 오염**되는데, 사람 개입 없이 자동 처리해 파이프라인 신뢰성을 지켰다.
...
| 데이터 분석 | Python, NumPy, Pandas, Matplotlib — 등급 통제 비교, 히트맵 |
